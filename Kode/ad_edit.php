<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && is_numeric($id)) {
        ...
        if ($ad_name && is_numeric($site_id) && is_numeric($zone_id)) {
            ...
            echo "hej4 <br>";
            $conn->query("UPDATE ads  SET 
                ad_name = '". $ad_name ."',
                file_name = '". $new_file_name ."', 
                max_impressions = '". $max_impressions ."', 
                ad_deadline = '". $timestamp ."', 
                site_id = '". $site_id ."', 
                zone_id = '". $zone_id ."' WHERE id = '". $id ."' ");
            
            header("location: ads.php");
        }
    } else {
        $ad_info = $conn->query("SELECT * FROM ads WHERE id = '". $id ."' ")->fetch_assoc();
        $sites = $conn->query("SELECT * FROM sites");
        $zones = $conn->query("SELECT * FROM zones WHERE site_id = '". $ad_info["site_id"] ."'");

        if($ad_info["ad_deadline"] == "0000-00-00 00:00:00") {
            $ad_deadline = "";
        } else {
            $ad_deadline = date("d/m/Y", strtotime($ad_info["ad_deadline"]));
        }
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <div class="widget">
                                                <div class="widget-header">
                                                    <h3><i class="fa fa-picture-o"></i> Edit ad</h3>
                                                </div>
                                                <div class="widget-content">                                             
                                                    <form role="form" method="post" action="ad_edit.php?id=<?php echo $id; ?>" enctype="multipart/form-data" id="ad_form">
                                                        <div class="form-group">
                                                            <label for="ad_name">Ad name *</label>
                                                            <input type="text" class="form-control" name="ad_name" id="ad_name" placeholder="Ad name" value=" <?php echo $ad_info["ad_name"]; ?> ">
                                                        </div>
                                                        
                                                        <div class="form-group">
                                                            <label for="max_impressions">Max impressions</label>
                                                            <input type="text" class="form-control" id="max_impressions" name="max_impressions" value=" <?php echo $ad_info["max_impressions"]; ?> ">
                                                        </div>
                                                        
                                                        <div class="form-group">
                                                            <label for="ad_deadline">Deadline for ad</label>
                                                            <input type="text" class="form-control" id="ad_deadline" name="ad_deadline" placeholder="DD/MM/YYYY" value=" <?php echo $ad_deadline; ?> ">
                                                        </div>
...
?>
                                                        ...
                                                        <br>
                                                        <button type="submit" class="btn btn-primary" id="create_submit"><i class="fa fa-check fa-inverse"></i> Save changes now</button>
                                                        <br>
                                                        <br>
                                                        <br>
                                                        * Required field
